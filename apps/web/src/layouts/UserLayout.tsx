...
// Profesional/Staff no ven Gastos ni Reportes (finanzas agregadas del
// negocio) — mismo criterio ya aplicado en las políticas de RLS de esas
// áreas. El link a Facturas, además de depender del plan
// (hasElectronicBilling, como ya era), tampoco se les muestra.
const OWNER_ADMIN_ONLY_PATHS = new Set(['/app/gastos', '/app/finanzas', '/app/facturas']);

function buildNavItems(hasElectronicBilling: boolean, currentRole: string | null): NavItem[] {
  let items = hasElectronicBilling
    ? (() => {
        const cobrosIndex = BASE_NAV_ITEMS.findIndex((item) => item.to === '/app/cobros');
        const withFacturas = [...BASE_NAV_ITEMS];
        withFacturas.splice(cobrosIndex + 1, 0, FACTURAS_NAV_ITEM);
        return withFacturas;
      })()
    : BASE_NAV_ITEMS;
  ...
  if (!isOwnerOrAdmin) {
    items = items.filter((item) => !OWNER_ADMIN_ONLY_PATHS.has(item.to));
  }
  return items;
}
...
function UserMenuPopoverItems({ onSelect, onSignOut, showConfiguracion }: { onSelect: () => void; onSignOut: () => void; showConfiguracion: boolean }) {
  return (
    <>
      {showConfiguracion && (
        <Link
...
function LayoutContent() {
  ...
  const navItems = buildNavItems(hasElectronicBilling, currentRole);
  const isOwnerOrAdmin = currentRole === 'owner' || currentRole === 'admin';
      ...
      <div className={`border-t border-slate-200 relative ${collapsed ? 'p-2' : 'p-4'}`} ref={userMenuRef}>
        ...
        {isUserMenuOpen && (
          collapsed && collapsedMenuCoords ? createPortal(
            ...
            >
              <UserMenuPopoverItems onSelect={() => { setIsUserMenuOpen(false); onNavClick?.(); }} onSignOut={handleSignOut} showConfiguracion={isOwnerOrAdmin} />
            </div>,
            document.body
          ) : (
